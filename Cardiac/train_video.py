...
from models.configs.transunet import vit_seg_configs as configs
from models.vivim import Vivim
from models.vivit import ViViTSegmentation
from models.pkechonet import PKEchoNet
from models.segformer import SegFormer
from models.net import ResNet18_FSTMamba_BSTAFusion, MobileNet_FSTMamba_BSTAFusion
from models.net import MobileViT125_FSTMamba_BSTAFusion, SegFormer_FSTMamba_BSTAFusion
...
def main():
    ...
    # ResNet18_STMamba / MobileViTv2_100_STMamba
    parser.add_argument('--modelname', default='ResNet18_FSTMamba_BSTAFusion', type=str, help='type of model, e.g., MemSAM, MedSAM, SAM, SAMFull, MSA, SAMed, SAMUS...')
    # echonet_video: 128, camus: 256, vivit: 224
    ...
    parser.add_argument('--sam_ckpt', type=str, default='MemSAM/checkpoints/sam_vit_b_01ec64.pth', help='Pretrained checkpoint of SAM')
    # batch_size: camus/8, echonet/16 // ResNet18_STMamba(16)
    parser.add_argument('--batch_size', type=int, default=4, help='batch_size per gpu') # MemSAM is 1. SAMed is 12 bs with 2n_gpu and lr is 0.005
    parser.add_argument('--n_gpu', type=int, default=1, help='total gpu')
    # base learning rate: default(1e-3), camus(1e-4) // ResNet18_STMamba(1e-3)
    parser.add_argument('--base_lr', type=float, default=3e-4, help='segmentation network learning rate, 0.005 for SAMed, 0.0001 for MSA') #0.0006
    parser.add_argument('--warmup', action="store_true", help='If activated, warp up the learning from a lower lr to the base_lr') 
    parser.add_argument('--warmup_period', type=int, default=250, help='Warp up iterations, only valid whrn warmup is activated')
    parser.add_argument('--keep_log', action="store_false", help='keep the loss&lr&dice during training or not')
    ...
    parser.add_argument('--disable_point_prompt', action="store_false")  # store_true for MemSAM
    ...
    #     int(args.encoder_input_size / args.vit_patches_size),
    #     int(args.encoder_input_size / args.vit_patches_size))
    # model = VisionTransformer(
    #     config_vit, img_size=args.encoder_input_size, num_classes=config_vit.n_classes)
    # model.load_from(weights=np.load(config_vit.pretrained_path))
    # model = SegFormer(num_classes=1)
    # model = Vivim(in_chans=3, out_chans=1)
    # model = PKEchoNet(in_channels=3, num_classes=1)
    model = ResNet18_FSTMamba_BSTAFusion(in_chan=3, n_classes=1, backbone='resnet18')
    # model = MobileViT125_FSTMamba_BSTAFusion(in_chan=3, n_classes=1, backbone='mobilevitv2_125')
    # model = MobileNet_FSTMamba_BSTAFusion(in_chan=3, n_classes=1, mode='mobilenetv3_large')
    # model = SegFormer_FSTMamba_BSTAFusion(in_chan=3, n_classes=1, mode='SegFormer')
    # model = ViViTSegmentation(in_chan=3, n_classes=1)
    ...
    val_dataset = EchoVideoDataset(opt.data_path, opt.test_split, tf_val, img_size=args.encoder_input_size,frame_length=args.frame_length, point_numbers=args.point_numbers, disable_point_prompt=args.disable_point_prompt)  # return image, mask, and filename
    ...
    if args.warmup:
        ...
        # optimizer = optim.AdamW(model.parameters(), lr=args.base_lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-2, amsgrad=False)
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (datapack) in enumerate(trainloader):
            ...
            # full supervised
                ...
                train_loss = criterion(pred[:,:,0], masks)  # camus-full
                # train_loss = criterion(pred[:,[0,-1],0], masks)  # echonet
                # train_loss = criterion(pred[:,[0,-1],0], masks[:,[0,-1]])  # camus-semi
            # -------------------------------------------------------- backward -------------------------------------------------------
            optimizer.zero_grad() 
            train_loss.backward()
            optimizer.step()
        ...
        if epoch % opt.save_freq == 0 or epoch == (opt.epochs-1):
            save_path = os.path.join(opt.save_path, args.modelname)
            if not os.path.isdir(save_path):
                os.makedirs(save_path)
            save_path = os.path.join(save_path, args.modelname + opt.save_path_code + '_' + str(epoch))
            torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)
            if args.keep_log:
