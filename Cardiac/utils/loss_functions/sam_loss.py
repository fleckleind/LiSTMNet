...
class BoundaryDOULoss(nn.Module):
    ...
    def _single_bdou_loss(self, score, target):
        h, w = 3, 3
        score, target = score.squeeze(1), target.squeeze(1)
        ...
        Y = torch.zeros((
            padding_out.shape[0], padding_out.shape[1] - h + 1, 
            padding_out.shape[2] - w + 1)).cuda()
        for i in range(Y.shape[0]):
            Y[i, :, :] = torch.conv2d(
                target[i].unsqueeze(0).unsqueeze(0), 
                kernel.unsqueeze(0).unsqueeze(0).cuda(), padding=1)
        Y = Y * target
        Y[Y == 5] = 0
    ...
    def forward(self, net_output, target, weight=None, sigmoid=False):
        if sigmoid:
            net_output = torch.sigmoid(net_output) # b 1 h w
        assert net_output.size() == target.size(), 'predict {} & target {} shape do not match'.format(net_output.size(), target.size())
        bdou_loss = self._single_bdou_loss(net_output[:, 0], target[:, 0])
        return bdou_loss
...
class Mask_DC_and_BCE_lossV3(nn.Module):
    def __init__(self, pos_weight, dice_weight=0.4, bdou_weight=0.3):
        """
        DO NOT APPLY NONLINEARITY IN YOUR NETWORK!
        THIS LOSS IS INTENDED TO BE USED FOR BRATS REGIONS ONLY
        :param soft_dice_kwargs:
        ...
        :param aggregate:
        """
        super(Mask_DC_and_BCE_lossV3, self).__init__()
    ...
    def forward(self, net_output, target):
        ...
        loss_ce = self.ce(low_res_logits, target)
        loss_dice = self.dc(low_res_logits, target, sigmoid=True)
        loss_bdou = self.bdou(low_res_logits, target, sigmoid=True)
        loss = (1 - self.dice_weight - self.bdou_weight) * loss_ce \
            + self.dice_weight * loss_dice + self.bdou_weight * loss_bdou
        return loss
...
def get_criterion(modelname='SAM', opt=None):
    ...
    if modelname == "SAMed":
        ...
        criterion = Mask_DC_and_BCE_lossV2(pos_weight=pos_weight)  # BCE+Dice
        # criterion = Mask_DC_and_BCE_lossV3(pos_weight=pos_weight)  # BCE+Dice+BDOU
    return criterion
