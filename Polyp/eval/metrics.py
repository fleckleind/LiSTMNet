...
class MedicalFixedSimple(object):
    def __init__(self, fixed_threshold=0.5):
    ...
    def step(self, pred, gt):
        # 二值化预测
        if pred.max() <= 1.0:
            pred_binary = pred > self.fixed_threshold
        else:
            pred_binary = (pred / 255.0) > self.fixed_threshold
        ...
        # 计算各个指标
        self.dice_scores.append(2 * tp / (2 * tp + fp + fn + eps))
        self.iou_scores.append(tp / (tp + fp + fn + eps))
        self.sensitivity_scores.append(tp / (tp + fn + eps))
        self.specificity_scores.append(tn / (tn + fp + eps))
...
class MedicalFixedSimpleImproved(object):
    def __init__(self, fixed_threshold=0.5, epsilon=1e-5):
        self.fixed_threshold = fixed_threshold
    ...
    def step(self, pred, gt):
        ...
        # 二值化预测
        if pred_max <= 1.0 + self.epsilon:
            pred_binary = pred > (self.fixed_threshold - self.epsilon)
        else:
            pred_normalized = pred / 255.0
            pred_binary = pred_normalized > (self.fixed_threshold - self.epsilon)
        
        # 二值化GT
        if gt_max > 1.0 + self.epsilon:
            gt_binary = gt > 128
        else:
            gt_binary = gt > 0.5
        ...
        if pred_binary.sum() == 0:
            print(f"警告: 预测全为0, pred范围 [{pred_min:.3f}, {pred_max:.3f}]")
        if gt_binary.sum() == 0:
            print(f"警告: GT全为0, gt范围 [{gt_min:.3f}, {gt_max:.3f}]")
    ...
    def check_consistency(self):
        """检查Dice和IoU的一致性"""
        for i, (dice, iou) in enumerate(zip(self.dice_scores, self.iou_scores)):
            theoretical_iou = dice / (2 - dice + self.epsilon)
            diff = abs(iou - theoretical_iou)
            if diff > 0.02:
                print(f"样本 {i}: Dice={dice:.4f}, IoU={iou:.4f}, "
                      f"理论IoU={theoretical_iou:.4f}, 差异={diff:.4f}")
                if i < len(self.debug_info):
                    print(f"  调试信息: {self.debug_info[i]}")